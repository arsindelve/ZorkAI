...
///     exchange as returned — question, text, and the <c>topic</c>/<c>rung</c> the response carried — and
///     replays them next time. That is how a hint ladder resumes where it left off without any server-side
///     state, regardless of which Lambda container answers.
/// </summary>
...
///     <see cref="Kind" /> is one of Progress, Mechanic, Lore, SoftLock, Grounded, Decline. For a Progress
///     rung, <see cref="Topic" /> and <see cref="Rung" /> must be echoed back in the next request's history;
///     <see cref="TotalRungs" /> lets a UI show "hint 2 of 3".
/// </summary>
public record HintApiResponse(
    string Text,
    string Kind = "Grounded",
    string? Topic = null,
    int Rung = 0,
    int TotalRungs = 0);
