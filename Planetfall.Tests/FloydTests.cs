...
public class FloydTests : EngineTestsBase
{
    ...
    public async Task Act_PerformsNormally_WhenNoPromptPending()
    {
        ...
        // Mock the chooser to pick a random action from constants (roll 6 picks from constant list)
        var mockChooser = new Mock<IRandomChooser>();
        mockChooser.Setup(r => r.RollDiceSuccess(20)).Returns(false); // Don't wander
        mockChooser.Setup(r => r.RollDice(12)).Returns(1); // Pass the 8.3% chance check
        mockChooser.Setup(r => r.RollDice(6)).Returns(6); // Pick random action from FloydConstants.RandomActions
    ...
    public async Task Act_WorksNormally_AfterFlagReset()
    {
        ...
        // Mock the Chooser to trigger constant action (roll 6 picks from constants)
        var mockChooser = new Mock<IRandomChooser>();
        mockChooser.Setup(r => r.RollDiceSuccess(20)).Returns(false); // Don't wander
        mockChooser.Setup(r => r.RollDice(12)).Returns(1); // Pass the 8.3% chance check
        mockChooser.Setup(r => r.RollDice(6)).Returns(6); // Pick random action from FloydConstants
        mockChooser.Setup(r => r.Choose(It.IsAny<List<string>>())).Returns((List<string> list) => list[0]);
        floyd.Chooser = mockChooser.Object;
