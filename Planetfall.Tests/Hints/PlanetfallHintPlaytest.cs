...
public class PlanetfallHintPlaytest : EngineTestsBase
{
    ...
    public async Task PlayTheOpening_AndInterrogateTheNarrator()
    {
        ...
        async Task<string> Hint(string checkpoint, string question)
        {
            ...
            // Echo kind/topic/rung exactly as the web client does: that is what advances the ladder.
            if (r.IsHint) history.Add(new HintExchange(question, r.Text, r.Topic, r.Rung, r.Kind.ToString()));
            log.AppendLine($"\n### {checkpoint}");
            log.AppendLine($"   [{r.Kind} topic={r.Topic ?? "-"} rung={r.Rung}/{r.TotalRungs}]");
