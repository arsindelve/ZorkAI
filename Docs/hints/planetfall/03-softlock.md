...
| 6 | **Survival clocks (hunger/thirst/sleep)** — running out kills the player | Death (preceded by a recoverable warning phase) | High | `Hunger`, `Tired` | Terminal threshold handled by `DeathProcessor` → restart. **The recoverable phase is a first-class hint category** — proactive eat/sleep nudges + on-demand "how do I handle being hungry/tired?", laddered from the invisiclues. See [01 § Survival-clock hints](01-puzzle-dag.md). |
