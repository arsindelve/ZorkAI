...
```
    cd "Stationfall-Lambda/src/Stationfall-Lambda"
    dotnet lambda deploy-serverless
```

## Deployment specifics for this stack

The `Deploy To AWS` GitHub workflow deploys this project with:

```
    dotnet lambda deploy-serverless -sb stationfallai-assets -sn StationfallStack --region us-east-1
```

which matches the Zork (`zorkai-assets` / `ZorkStack`) and Planetfall (`planetfallai-assets` /
`PlanetfallStack`) stacks.

Two things the deploy depends on are NOT created by this template, and already exist in the AWS
account (`576431164672`, us-east-1):

* the S3 staging bucket `stationfallai-assets`. Unlike the older `planetfallai-assets` bucket, it
  keeps S3's modern default of Block Public Access fully on — it only ever holds Lambda deployment
  zips read in-account by CloudFormation, so nothing about it needs to be publicly reachable.
