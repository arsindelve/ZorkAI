...
internal class SimpleInteractionEngine : IIntentEngine
{
    public async Task<string> Process(IntentBase intent, IContext context, IGenerationClient generationClient)
    {
        ...
        InteractionResult contextInteraction = context.RespondToSimpleInteraction(simpleInteraction);
        ...
        if (contextInteraction is NoVerbMatchInteractionResult noVerbContext)
            return await GetGeneratedNoMatchingVerbResponse(noVerbContext.Noun, noVerbContext.Verb, generationClient, context);
    ...
    private static async Task<string> GetGeneratedNoMatchingVerbResponse(string? noun, string verb, IGenerationClient generationClient, IContext context)
    {
        var request =
    ...
    private static async Task<string> GetGeneratedNounNotPresentResponse(string? noun, IGenerationClient generationClient, IContext context)
    {
    ...
    private static async Task<string> GetGeneratedNoOpResponse(string input, IGenerationClient generationClient, IContext context)
    {
        var request =
