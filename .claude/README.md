...
**This file is automatically generated** by the MSBuild hook in `Directory.Build.targets` after every `dotnet build`.
...
The repository's `Directory.Build.targets` file includes a post-build hook that:
1. Runs after every `dotnet build` (CLI, IDE, or automatic)
...
1. **Run build once to generate the cache:**
   ```bash
   dotnet build
   ```
...
- Run `dotnet build`
- Build from your IDE (Visual Studio, Rider, VS Code)
- After adding/updating/removing packages and rebuilding
